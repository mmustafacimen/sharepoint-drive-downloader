...
def download_items(access_token, parent_path="root", local_path="."):
    drive_url = f"https://graph.microsoft.com/v1.0/me/drive/{parent_path}/children"
    headers = {
    ...
    if response.status_code == 200:
        ...
        for item in items:
            ...
            if 'file' in item:
                ...
                download_items(access_token, parent_path=f"items/{item['id']}", local_path=item_path)
    else:
        print(f"Error: {response.status_code}, {response.text}")

def main():
    access_token = get_access_token(username, password)
    download_items(access_token)
